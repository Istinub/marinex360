...
// Read-only public tracking is intentionally looser than guest job-request writes.
export const PUBLIC_JOB_PROGRESS_RATE_LIMIT = {
  maxRequests: 600,
  windowSeconds: 10 * 60,
} as const;

const REDIS_KEY_PREFIX = 'marinex360:rate-limit:job-progress-public';
...
export function publicJobProgressRateLimitKey(ip: string): string {
...
function parseHit(value: unknown): RateLimitHit {
  ...
  return {
    count,
    ttlSeconds: Number.isFinite(ttl) && ttl > 0 ? Math.ceil(ttl) : PUBLIC_JOB_PROGRESS_RATE_LIMIT.windowSeconds,
  };
}

export function createPublicJobProgressRateLimiter(): {
  ...
  async function hit(ip: string): Promise<RateLimitHit> {
    return parseHit(await client().eval(
      ...
      1,
      publicJobProgressRateLimitKey(ip),
      String(PUBLIC_JOB_PROGRESS_RATE_LIMIT.windowSeconds),
    ));
  }
  ...
  return {
    async preHandler(req, reply): Promise<void> {
      ...
      const remaining = Math.max(0, PUBLIC_JOB_PROGRESS_RATE_LIMIT.maxRequests - result.count);
      reply
        .header('X-RateLimit-Limit', String(PUBLIC_JOB_PROGRESS_RATE_LIMIT.maxRequests))
        .header('X-RateLimit-Remaining', String(remaining))
        .header('X-RateLimit-Reset', String(Math.ceil(Date.now() / 1000) + result.ttlSeconds));

      if (result.count <= PUBLIC_JOB_PROGRESS_RATE_LIMIT.maxRequests) return;
